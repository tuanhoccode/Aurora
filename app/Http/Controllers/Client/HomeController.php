    ...
    public function shop()
{
    ...
    // Tối ưu query featured products, bao gồm variants để hiển thị regular_price
    $featuredThisWeek = Product::select('id', 'name', 'slug', 'price', 'thumbnail', 'views', 'is_active', 'type')
        ->with(['variants' => function ($q) {
            $q->select('id', 'product_id', 'regular_price')
              ->take(1); // Chỉ lấy variant đầu tiên để tối ưu
        }])
        ->where('is_active', 1)
