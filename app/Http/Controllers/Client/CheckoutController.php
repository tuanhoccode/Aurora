...
class CheckoutController extends Controller
{
    ...
    public function process(Request $request)
    {
        try {
            ...
            // Lưu thông tin đơn hàng tạm thời vào session cho VNPay
            $orderData = [
                'user_id' => Auth::id(),
    ...
    protected function createOrder(array $orderData)
    {
        $order = Order::create([
            ...
            'payment_id' => $orderData['payment_method'] ?? 'cod' === 'cod' ? 1 : 2,
    ...
    protected function clearCartAndSession($cart, $cartItems)
    {
        ...
        session()->forget(['coupon', 'selected_items', 'pending_order']);
    }
    ...
    protected function vnpayPayment(array $orderData)
    {
        try {
            ...
            $vnp_TxnRef = Str::upper('ORD-' . Str::random(8)); // Tạo mã tạm thời
