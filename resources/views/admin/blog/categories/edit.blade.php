                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-select @error('parent_id') is-invalid @enderror" name="parent_id">
                                        ...
                                        @foreach($parentCategories as $parent)
                                            @if($parent->id != $category->id) {{-- Ngăn chọn chính nó làm cha --}}
...
<script>
    $(document).ready(function() {
        ...
        $('#remove_image').on('change', function() {
            if ($(this).is(':checked')) {
                $('#image-preview').attr('src', '{{ asset('images/default-placeholder.png') }}');
            } else {
                $('#image-preview').attr('src', '{{ $category->image ? asset($category->image) : asset('images/default-placeholder.png') }}');
            }
        });
    });
</script>
