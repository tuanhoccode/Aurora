                        ...
                        <tr>
                            ...
                            <td>
                                {{-- @if ($brand->logo) --}}
                                    <img src="{{ asset('storage/' . $brand->logo) }}" alt="Logo"
                                        style="max-width:80px;" class="rounded">
                                {{-- @else
                                    N/A
                                @endif --}}
                            </td>
                            ...
                            <td>
                                <a href="{{ route('admin.brands.show', $brand->id) }}"
                                    class="btn btn-info btn-sm shadow-sm rounded" title="View">
                                    <i class="bi bi-eye"></i>
                                </a>
                                ...
                                @if ($brand->deleted_at)
                                    <form action="{{ route('admin.brands.forceDelete', $brand->id) }}" method="POST"
                                        style="display:inline-block"
                                        onsubmit="return confirm('Force delete? This cannot be undone!');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-dark btn-sm shadow-sm rounded"
                                            title="Force Delete">
