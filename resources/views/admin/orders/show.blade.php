...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h3 mb-0 fw-bold text-gray-800">Chi tiết đơn hàng</h1>
            <p class="text-muted mt-1">Thông tin chi tiết đơn hàng trong hệ thống</p>
        </div>
        <div>
            <a href="{{ route('admin.orders.index') }}" class="btn btn-light rounded-pill shadow-sm"><i class="fas fa-arrow-left me-1"></i> Quay lại danh sách</a>
        </div>
    </div>
    @if (session('success'))
    ...
    <div class="row">
        <div class="col-lg-8 mb-4">
            <div class="card shadow-sm rounded mb-4">
                <div class="card-header bg-light fw-bold"><i class="fas fa-shopping-cart me-2"></i>Đơn hàng #{{ $order->code }}</div>
                <div class="card-body">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if ($item->product && $item->product->thumbnail)
                                                <img src="{{ asset('storage/' . $item->product->thumbnail) }}" alt="{{ $item->name }}" style="width: 50px; height: 50px; object-fit: cover;">
                                            @else
                                                <span class="text-muted"><i class="fas fa-ban me-1"></i> Không có ảnh</span>
                                            @endif
                                        </td>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <div><i class="fas fa-user me-1"></i> <strong>Khách hàng:</strong> {{ $user->fullname ?? 'Không xác định' }}</div>
                                <div><i class="fas fa-envelope me-1"></i> <strong>Email:</strong> {{ $user->email ?? 'Không xác định' }}</div>
                                <div><i class="fas fa-map-marker-alt me-1"></i> <strong>Địa chỉ:</strong> {{ $user->address->address ?? 'Không xác định' }}</div>
                                <div><i class="fas fa-phone me-1"></i> <strong>Điện thoại:</strong> {{ $user->phone_number ?? 'Không xác định' }}</div>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="mb-3">
                                <div class="fw-bold mb-2"><i class="fas fa-truck me-2"></i>Chi tiết giao hàng</div>
                                <div><i class="fas fa-user me-1"></i> <strong>Khách hàng:</strong> {{ $order->fullname ?? 'Không xác định' }}</div>
                                <div><i class="fas fa-envelope me-1"></i> <strong>Email:</strong> {{ $order->email ?? 'Không xác định' }}</div>
                                <div><i class="fas fa-phone me-1"></i> <strong>Điện thoại:</strong> {{ $order->phone_number ?? 'Không xác định' }}</div>
                                <div><i class="fas fa-map-marker-alt me-1"></i> <strong>Địa chỉ:</strong> {{ $order->address ?? 'Không xác định' }}</div>
                                <div><i class="fas fa-shipping-fast me-1"></i> <strong>Phương thức vận chuyển:</strong> Giao hàng {{ $order->shipping_type }}</div>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <table class="table table-bordered mb-0">
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @elseif ($order->shipping_type == 'nhanh' || $order->shipping_type == 'Oto')
            ...
            <div class="card shadow-sm rounded mb-4">
                <div class="card-header bg-light fw-bold"><i class="fas fa-info-circle me-2"></i>Trạng thái đơn hàng</div>
                <div class="card-body">
                    <div><i class="fas fa-credit-card me-1"></i> <strong>Trạng thái thanh toán:</strong> {{ $paymentStatus }}</div>
                    <div><i class="fas fa-check-square me-1"></i> <strong>Trạng thái hoàn thành:</strong> {{ $fulfillmentStatus }}</div>
                </div>
            </div>
            <div class="card shadow-sm rounded mb-4">
                ...
                <div class="card-body p-0">
                    @if ($order->statusHistory->isEmpty())
                        <p class="text-muted p-3"><i class="fas fa-exclamation-triangle me-1"></i> Chưa có lịch sử trạng thái.</p>
                    @else
        ...
        <div class="col-lg-4 mb-4">
            <div class="card shadow-sm rounded mb-4">
                <div class="card-header bg-light fw-bold"><i class="fas fa-sync-alt me-2"></i>Cập nhật trạng thái đơn hàng</div>
                <div class="card-body">
                    <form action="{{ route('admin.orders.updateStatus', $order->id) }}" method="POST">
                        ...
                        <button type="submit" class="btn btn-primary w-100 mb-2"><i class="fas fa-save me-1"></i> Cập nhật</button>
                        <a href="{{ route('admin.orders.index') }}" class="btn btn-secondary w-100"><i class="fas fa-arrow-left me-1"></i> Quay lại</a>
                    </form>
                </div>
            </div>
        </div>
