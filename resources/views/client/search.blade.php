...
<div class="search-title text-capitalize fw-normal text-dark" style="color: #1E3A8A;">Kết quả tìm kiếm:
    {{ mb_strtoupper($query) }}</div>
@php
    ...
    @if (!$isOutOfStock)
        ...
        <div class="col-xl-3 col-lg-4 col-md-6 col-sm-6 mb-4 d-flex">
            <div class="tp-product-item-2 mb-40 search-result w-100">
                <div
                    class="tp-product-thumb-2 p-relative z-index-1 fix w-img @if ($isOutOfStock) out-of-stock @endif">
                    @if ($product->slug)
                    ...
                    <div class="tp-product-price-wrapper-2">
                        @php
                            if ($product->type === 'variant' && $product->variants->count()) {
                                ...
                                $variantWithStock = $product->variants->where('stock', '>', 0);

                                // Giá thấp nhất và cao nhất (sale_price nếu có, còn không lấy regular_price)
                                $minPrice =
                                    $variantWithStock->min(function ($v) {
                                        return $v->sale_price > 0 ? $v->sale_price : $v->regular_price;
                                    }) ?? 0;

                                $maxPrice =
                                    $variantWithStock->max(function ($v) {
                                        return $v->sale_price > 0 ? $v->sale_price : $v->regular_price;
                                    }) ?? 0;
                            } else {
                                // Sản phẩm đơn
                            ...
                            @if ($product->type === 'variant')
                                @if ($minPrice === $maxPrice)
                                    <span class="tp-product-price-2 new-price"
                                        ...
                                        {{ number_format($minPrice, 0, ',', '.') }}<span style="color:red;">
                                            đ</span>
                                    </span>
                                @else
                                    <span class="tp-product-price-2 new-price"
                                        ...
                                        {{ number_format($maxPrice, 0, ',', '.') }}<span style="color:red;">
                                            đ</span>
                                    </span>
                                @endif
                            @else
                                ...
                                @if ($product->sale_price > 0 && $product->price > $product->sale_price)
                                    <span class="tp-product-price-2 old-price"
                                        style="color:#888; font-size:0.9rem; margin-left:4px; text-decoration:line-through;">
                                        {{ number_format($maxPrice, 0, ',', '.') }}<span style="color:red;">
                                            đ</span>
                                    </span>
                                @endif
