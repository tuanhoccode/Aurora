...
<style>
    ...
    .sp-card{border:1px solid #eee; border-radius:2px; overflow:hidden; background:#fff}
    .sp-head{display:flex; align-items:center; justify-content:space-between; gap:12px; padding:12px 16px; border-bottom:1px solid #f0f0f0}
    .sp-like{background:#ee4d2d; color:#fff; font-size:12px; font-weight:700; padding:4px 8px; border-radius:2px}
    .sp-head .shop{font-weight:700; margin-left:8px}
    .sp-mini{border:1px solid #ddd; background:#fff; border-radius:2px; padding:6px 10px; font-size:12px; cursor:pointer; margin-left:6px}
    .sp-ok{color:#00bfa5; display:flex; align-items:center; gap:6px}
    .sp-done{color:#ee4d2d; font-weight:700}
    .sp-row{display:flex; gap:12px; padding:14px 16px; border-bottom:1px solid #f0f0f0}
    .sp-thumb{width:80px; height:80px; object-fit:cover; border:1px solid #eee; border-radius:2px}
    .sp-title{font-size:16px; color:#333; margin:2px 0 6px}
    .sp-variant,.sp-qty{font-size:13px; color:#7a7a7a; margin-top:2px}
    .sp-price{text-align:right; min-width:140px; white-space:nowrap}
    .sp-old{color:#9e9e9e; text-decoration:line-through; margin-right:8px}
    .sp-new{color:#ee4d2d; font-weight:700}
    .sp-foot{background:#fff7f2; display:flex; justify-content:flex-end; align-items:center; gap:12px; padding:16px}
    .sp-total{font-size:16px}
    .sp-total b{color:#ee4d2d; font-size:22px}
    .sp-btn{border-radius:2px; padding:10px 20px; font-weight:700; cursor:pointer; border:1px solid transparent; font-size:14px}
    .sp-btn-primary{background:#ee4d2d; border-color:#ee4d2d; color:#fff}
    .sp-btn-ghost{background:#fff; border:1px solid #dcdcdc; color:#333}
    @media (max-width:576px){ .sp-row{flex-wrap:wrap} .sp-price{width:100%; text-align:left} }
</style>
                ...
                @foreach($orders as $order)
                    @php
                        ...
                        $shopName = data_get($firstItem, 'product.store.name')
                            ...
                            ?? data_get($firstItem, 'product.user.name')
                            ?? 'Cửa hàng';
                    @endphp
                    ...
                    <div class="sp-card mb-4">
                        ...
                        <div class="sp-foot">
                            <div class="sp-total">Thành tiền: <b>{{ number_format($order->total_amount, 0, ',', '.') }} ₫</b></div>
...
<style>
    ...
    /* Nút hành động */
    .btn-detail, .btn-track, .btn-cancel-order {
    ...
    .bg-refunded { background-color: #e2e3e5; color: #383d41; }
</style>
@endpush
