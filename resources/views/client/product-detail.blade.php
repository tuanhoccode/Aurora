...
@section('content')
    <style>
        ...
        /* Đặt nút nằm giữa chiều cao ảnh và không tràn */
        .tp-product-related-slider-active {
            position: relative;
            padding: 0 20px;
            /* tạo không gian cho nút nằm gọn */
        }
                                        ...
                                        <div class="tp-product-details-action-item-wrapper d-flex align-items-center">
                                            <div class="tp-product-details-quantity">
                                                <div class="tp-product-quantity mb-15 mr-15">
                                                    ...
                                                    <input id="quantity" class="tp-cart-input" type="text"
                                                        value="1">
                                                    <span class="tp-cart-plus">+</span>
                                                </div>
                                            </div>
                                            <div class="tp-product-details-add-to-cart mb-15 w-100">
                                                <button class="tp-product-details-add-to-cart-btn w-100">Thêm vào giỏ
                                                    hàng</button>
                                            </div>
                                        </div>
                                                            ...
                                                            <tbody>
                                                                @foreach ($product->attributeValues as $attrValue)
                                                                    @if ($attrValue->attribute)
                                                                        <tr>
                                                                            <td>{{ $attrValue->attribute->name }}</td>
                        ...
                        <div class="swiper tp-product-related-slider-active">
                            <div class="swiper-wrapper">
                                @forelse ($relatedByCategory as $related)
                                    <div class="swiper-slide">
                                        <div class="tp-product-item-3 tp-product-style-primary mb-20 p-3 shadow-sm rounded"
                                            style="border: 1px solid #eee;">
                                            <div class="tp-product-thumb-3 mb-15 fix p-relative z-index-1 text-center">
                                                <a href="{{ route('client.product.show', $related->slug) }}">
                                                    <img src="{{ asset('storage/' . ($related->thumbnail ?? ($related->img ?? 'images/no-image.jpg'))) }}"
                                                        alt="{{ $related->name }}"
                                                        style="max-height: 160px; object-fit: contain;">
                                                </a>
                                            </div>

                                            <div class="tp-product-content-3 text-center">
                                                <div class="tp-product-tag-3 mb-1 text-muted" style="font-size: 13px;">
                                                    @foreach ($related->categories as $cat)
                                                        <span>{{ $cat->name }}</span>
                                                    @endforeach
                                                </div>
                                                <h3 class="tp-product-title-3 mb-1" style="font-size: 15px;">
                                                    <a href="{{ route('client.product.show', $related->slug) }}">
                                                        {{ $related->name }}
                                                    </a>
                                                </h3>
                                                <div class="tp-product-price-wrapper-3">
                                                    <span class="tp-product-price-3 new-price text-danger fw-bold">
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            document.querySelector('.tp-cart-minus')?.addEventListener('click', () => {
                let val = parseInt(quantityInput.value) || 1;
                if (val > 1) quantityInput.value = val - 1;
                updateTotal();
            });

            document.querySelector('.tp-cart-plus')?.addEventListener('click', () => {
                let val = parseInt(quantityInput.value) || 1;
