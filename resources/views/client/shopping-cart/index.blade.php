                        ...
                        <div class="cart-items-grid">
                            ...
                            @foreach ($cartItems as $item)
                                ...
                                <div class="cart-item-card" data-item-id="{{ $item->id }}"
                                    data-unit-price="{{ $unitPrice }}" data-stock="{{ $stock }}">
                                    ...
                                    <div class="cart-item-card__info">
                                        ...
                                        <div class="meta-attributes">
                                            <div class="meta" title="{{ $variant->sku ?? $product->sku }}">
                                                <strong>Mã:</strong> {{ $variant->sku ?? $product->sku }}</div>
                                            @if ($variant)
                                                @php
                                                ...
                                                @endphp
                                                <div class="meta" title="{{ $size ?? 'N/A' }}"><strong>Size:</strong>
                                                    {{ $size ?? 'N/A' }}</div>
                    ...
                    <div class="col-lg-4 offset-lg-1">
                        <div class="cart-summary-box">
                            ...
                            <div class="cart-summary__item">
                                <span>Tạm tính (<span class="cart-item-count">{{ $cartItems->sum('quantity') }}</span> sản
                                    phẩm)</span>
                            ...
                            <div class="cart-summary__coupon my-4">
                                <a href="#coupon-form" class="coupon-toggle" data-bs-toggle="collapse" aria-expanded="false"
                            ...
                            <div class="cart-summary__total">
                                <span>Tổng cộng</span>
                                <span
                                    id="cart-grand-total">{{ isset($cartTotal) ? number_format($cartTotal + 20000, 0, ',', '.') : '0' }}₫</span>
                            </div>
                            <a href="{{ route('checkout') }}" class="checkout-btn">Tiến hành thanh toán <i
...
@section('scripts')
    <script>
        ...
        function changeCartQty(itemId, value) {
            ...
            updateCartQty(itemId, newQty - parseInt(document.querySelector('.qty-input[data-item-id="' + itemId + '"]')
                .value));
        }
