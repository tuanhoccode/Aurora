...
@section('content')
    ...
    <div class="container mt-5">
        <div class="row">
            <div class="col-12">
                <h2 class="mb-4 mt-2" style="font-weight:700;font-size:2.2rem;line-height:1.2;">Hiển thị danh mục:
                    {{ $category->name }}</h2>
            </div>
        </div>
                    ...
                    <form method="GET" action="{{ route('client.categories.show', $category->id) }}">
                                        ...
                                        <li class="d-flex justify-content-between mb-10">
                                            <div class="form-check">
                                                <input class="form-check-input" type="checkbox" name="is_sale"
                                                    value="1" id="is_sale" {{ request('is_sale') ? 'checked' : '' }}>
                                            ...
                                            <li class="mb-2">
                                                <div class="form-check">
                                                    <input class="form-check-input" type="checkbox" name="price_ranges[]"
                                                        ...
                                                        id="price_range_{{ $i }}"
                                                        {{ collect(request('price_ranges'))->contains($range['min'] . '-' . $range['max']) ? 'checked' : '' }}>
                                                    <label class="form-check-label"
                                                        for="price_range_{{ $i }}">{{ $range['label'] }}</label>
                                                </div>
                                            </li>
                                        @endforeach
                                    </ul>
                        ...
                        @if (isset($variantAttributes) && $variantAttributes->count() > 0)
                            @foreach ($variantAttributes as $attribute)
                                @php
                                    $isColorAttribute = in_array(strtolower(trim($attribute->name)), [
                                        'màu sắc',
                                        'màu',
                                        'color',
                                        'mau',
                                                ...
                                                <div class="d-flex flex-wrap gap-2">
                                                    @foreach ($attribute->attributeValues as $value)
                                                        @php
                                                            $isActive =
                                                                is_array(
                                                                    request('variant_attributes.' . $attribute->id),
                                                                ) &&
                                                                in_array(
                                                                    $value->id,
                                                                    request('variant_attributes.' . $attribute->id, []),
                                                        ...
                                                        <div class="color-option {{ $isActive ? 'active' : '' }}"
                                                            ...
                                                            <div class="color-swatch" style="{{ $colorStyle }}">
                                                                ...
                                                                <span class="checkmark"
                                                                    style="display: {{ $isActive ? 'block' : 'none' }}; color: white; text-align: center; line-height: 26px;">✓</span>
                                                            </div>
                                                        </div>
                                                    @endforeach
                                                </div>
                                                    ...
                                                    @foreach ($attribute->attributeValues as $value)
                                                        <li class="d-flex justify-content-between mb-2">
                                                            <div class="form-check">
                                                                ...
                                                                <label class="form-check-label"
                                                                    for="variant_attr_{{ $attribute->id }}_{{ $value->id }}">
                                                                    {{ $value->value }}
                                                                </label>
                                                            </div>
                                                        </li>
            ...
            <div class="col-xl-9 col-lg-9">
                    ...
                    @foreach ($products as $index => $product)
                        ...
                        <div class="col-xl-4 col-lg-4 col-md-6 col-sm-6 product-item{{ $index >= $defaultShow ? ' d-none-by-js' : '' }}"
                            ...
                            <div class="tp-product-item-2">
                                <div
                                    class="tp-product-thumb-2 p-relative z-index-1 fix w-img {{ $isOutOfStock ? 'out-of-stock' : '' }}">
                                    <a
                                        href="{{ $product->slug ? route('client.product.show', ['slug' => $product->slug]) : '#' }}">
                                    ...
                                    @php
                                        ...
                                        if ($isVariant) {
                                            $variantPrices = $product->variants
                                                ->map(function ($v) {
                                                    $base = $v->regular_price ?? $v->price; // ưu tiên regular_price
                                                    $effective =
                                                        $v->sale_price !== null && $v->sale_price > 0
                                                            ? $v->sale_price
                                                            : $base;
                                                    return is_numeric($effective) ? (float) $effective : null;
                                                })
                                                ->filter(fn($p) => !is_null($p) && $p > 0); // tránh 0đ / null

                                            if ($variantPrices->isNotEmpty()) {
                                                $minPrice = $variantPrices->min();
                                                $maxPrice = $variantPrices->max();
                                            } else {
                                                // fallback cuối cùng: dùng giá sản phẩm nếu có
                                                $minPrice = $maxPrice = (float) ($product->price ?? 0);
                                            }
                                        } else {
                                            $base = $product->price ?? null;
                                            $sale =
                                                $product->sale_price !== null && $product->sale_price > 0
                                                    ? $product->sale_price
                                                    : null;
                                            $minPrice = (float) ($sale ?? ($base ?? 0));
                                            $maxPrice = (float) ($base ?? 0);
                                        }
                                    @endphp
                ...
                <div class="text-center my-4">
                    <div id="showing-info" class="mb-2">
                        Đang hiển thị <span id="showing-count">{{ min($defaultShow, $products->count()) }}</span> trong số
            ...
            if (filterForm) {
                filterForm.addEventListener('submit', function(e) {
                    ...
                    if (submitBtn) {
                        ...
                        submitBtn.innerHTML =
                            '<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Đang lọc...';
                    }
                });
            }
        });
